...
from microninja.utils import read_json, write_json, get_date_now, ensure_dir, \
    chown_path, run_print_output_error, run_bg
from microninja.logging import logger
from microninja_profile.paths import apps_dir, xp_file, microninjaprofile_dir, \
    app_profiles_file
...
def save_app_state(app_name, data):
    ...
    if 'SUDO_USER' in os.environ:
        chown_path(microninjaprofile_dir)
        chown_path(apps_dir)
...
def get_all_users():
    ...
    for user in possible_users:
        profile_path = os.path.join("/home", user, ".microninjaprofile")
...
def save_app_state_variable_all_users(app, variable, value):
    ...
    for user in users:
        dir_path = os.path.join(
            "/home", user, ".microninjaprofile", "apps", app
        )
