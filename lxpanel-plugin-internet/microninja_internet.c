...
#define NO_INTERNET_ICON "/usr/share/microninja-settings/icon/widget-no-internet.png"
#define WIFI_ICON "/usr/share/microninja-settings/icon/widget-wifi.png"
#define WIFI_SETTING_ICON "/usr/share/microninja-settings/media/Icons/Icon-Wifi.png"

#define INTERNET_STATUS_FILE "/var/opt/internet_monitor"

#define INTERNET_CMD "/usr/bin/is_internet"
#define SETTINGS_CMD "sudo /usr/bin/microninja-settings 4"
#define WIFI_CMD "sudo /usr/bin/microninja-wifi-gui"
#define RECONNECT_CMD "sudo /usr/bin/microninja-connect -c wlan0"
#define SOUND_CMD "/usr/bin/aplay /usr/share/microninja-media/sounds/microninja_open_app.wav"
//#define PLUGIN_TOOLTIP "Stato della connessione internet"
#define DISCONNECT_CMD "sudo /usr/bin/microninja-wifi-gui --disconnect"
...
static GtkWidget *plugin_constructor(LXPanel *panel, config_setting_t *settings)
{
    ...
    setlocale (LC_ALL, "");
    bindtextdomain ("microninja_settings_plugins", "/usr/share/locale/microninja/microninja-settings/");
    textdomain ("microninja_settings_plugins");
    ...
    gtk_tooltips_set_tip(tooltips, GTK_WIDGET(icon), _("Internet connection status"), NULL);
...
static gboolean internet_status(kano_internet_plugin_t *plugin) {
    ...
    if (internet == 0) {
        ...
        // If there is no internet try to run microninja-connect to reconnect
        ...
        // run microninja-connect if everything was OK
...
{
    /* Launch settings*/
    launch_cmd(WIFI_CMD, "microninja-wifi-gui");
    /* Play sound */
...
void disconnect_clicked(GtkWidget *widget)
{
    ...
    launch_cmd(DISCONNECT_CMD, "microninja-wifi-gui");
    /* Play sound */
    launch_cmd(SOUND_CMD, NULL);
}
...
static gboolean show_menu(GtkWidget *widget, GdkEventButton *event, kano_internet_plugin_t *plugin)
{
    ...
    if (strcmp(internet, "NOT CONNECTED") == 0) {
        ...
        header_item = gtk_menu_item_new_with_label(_("Not connected"));
        ...
        GtkWidget *header_item = gtk_menu_item_new_with_label(_("Connected"));
        gtk_widget_set_sensitive(header_item, FALSE);
        gtk_menu_append(GTK_MENU(menu), header_item);
        gtk_widget_show(header_item);
        ...
        if (strcmp(internet, "WIRELESS") == 0) {
            ...
            GtkWidget *disconnect_item = gtk_menu_item_new_with_label(_("Disconnect"));
...
FM_DEFINE_MODULE(lxpanel_gtk, microninja_internet)

/* Plugin descriptor. */
LXPanelPluginInit fm_module_init_lxpanel_gtk = {
    .name = "Microninja Internet",
    .description = "Shows the status of your internet connection.",
