...
from microninja_settings.boot_config import set_config_value, get_config_value
from microninja.logging import logger
from microninja_settings.config_file import set_setting
...
CLOCK_MODES = {
    ...
    CLOCK_RPI1: {
        'modes': ['Nessuno', 'Modesto', 'Medio', 'Alto', 'Turbo'],
        'default': 'Alto',
        'warning': ['Turbo'],
        'values': {
            'Nessuno': {
    ...
    CLOCK_RPI2: {
        'modes': ['Predefinito', 'Overcloccato'],
        'default': 'Predefinito',
        'warning': ['Overcloccato'],
        'values': {
            'Predefinito': {
                'arm_freq': 900,
                'core_freq': 250,
...
def change_overclock_value(config, is_pi2):
    try:
        ...
        logger.error(
            'microninja-settings: set_overclock: SetOverclock: set_overclock(): '
