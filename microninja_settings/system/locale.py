...
import re
import os
from microninja.utils import run_cmd, read_file_contents, sed, enforce_root, \
...
XSESSION_RC_FILE = os.path.join(get_home_by_username(get_user_unsudoed()),
                                '.xsessionrc')
CACHE_MENU_PATH = os.path.join(get_home_by_username(get_user_unsudoed()),
                                '.cache/menus/')
...
def install_locale(locale):
    ...
    enforce_root(_('You need to be root to install a locale'))
...
def set_locale(locale):
    ...
    run_cmd('sudo rm -rf ' + CACHE_MENU_PATH)
