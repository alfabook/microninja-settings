...
from microninja.network import is_internet, network_info, launch_browser
from microninja_settings.system.proxy import get_all_proxies, set_all_proxies, test_proxy


class SetWifi(Template):
    wifi_connection_attempted = False

    def __init__(self, win, noInternet = False):
        ...
        if not noInternet:
            ...
            self.kano_button.connect("button-release-event", Gtk.main_quit)
        ...
        if not common.has_internet or not network_info_dict:
            if network_info_dict:
                description = _("Use your browser to perform login or configure proxy")
            else:
            ...
            internet_status.set_text(_("No networks found"))
            ...
            self.kano_button.set_label(_("COMPLETED"))
    ...
    def go_to_proxy(self, widget, event):
        self.win.clear_win()
        SetProxy(self.win, self.noInternet)
    ...
    def configure_wifi(self, widget=None, event=None):
        ...
        if not hasattr(event, 'keyval') or event.keyval == Gdk.KEY_Return:
            ...
            # Call WiFi config
            os.system('/usr/bin/microninja-wifi-gui')

            # Refresh window after WiFi Setup
            self.win.unblur()
            self.win.clear_win()
            SetWifi(self.win, self.noInternet)
...
class SetProxy(Gtk.Box):
    def __init__(self, win, noInternet = False):
        ...
        self.username_entry.props.placeholder_text = _("User")
            ...
            def lengthy_process():
                ...
                if self.enable_proxy:
                    ...
                    if not success:
                        ...
                        title = _("Proxy successfully enabled")
                        description = ""
                        return_value = 0
                    ...
                    set_all_proxies(False)
                    common.proxy_enabled = False
                    title = _("Proxy successfully disabled")
                    description = ""
                    return_value = 0
