...
from microninja.gtk3.buttons import KanoButton, OrangeButton
...
class Template(Gtk.Box):
    ...
    def __init__(
        ...
        self.kano_button.set_size_request(-1, 44)
        self.kano_button.align.set_padding(0, 30, 0, 0)
        
        if close_btn:
            self.kano_button_close = KanoButton(_("CLOSE"))
            self.kano_button_close.pack_and_align()
            self.kano_button_close.set_size_request(-1, 44)
            self.kano_button_close.align.set_padding(0, 30, 0, 0)
        ...
        if not close_btn:
            ...
            leftBox = Gtk.Box()
            leftBox.set_size_request(100, -1)
            rightBox = Gtk.Box()
            rightBox.set_size_request(100, -1)
            hBox.pack_start(leftBox, False, False, 0)
            hBox.pack_start(self.kano_button.align, False, False, 0)
            hBox.pack_end(rightBox, False, False, 0)  
            hBox.pack_end(self.kano_button_close.align, False, False, 0)  
...
class EditableList(Gtk.Grid):
    ...
    def _item_edited_handler(self, cellrenderertext, path, new_text):
        if new_text is None:
            ...
            title = _('Not a valid website')
            description = _("\nWe must be sure that the URL website is valid.\n" 
                          "Please, insert the url website how it appears in your browser.\n\n" 
                          "Example: http://www.google.com\n")
            buttons = "OK:red:1"
            cmd = 'microninja-dialog title="{}" description="{}" buttons="{}" no-taskbar &'.format(
