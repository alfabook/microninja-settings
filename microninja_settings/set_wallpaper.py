...
# removed create_wallpaper_dict
# Italian translation

import os
from gi.repository import Gtk, GdkPixbuf, Gdk
from microninja_settings.config_file import set_setting, get_setting
#from microninja_profile.badges import calculate_badges
from microninja_settings.config_file import username
from microninja_settings.system.wallpaper import change_wallpaper
from microninja_settings.image_table import ImageTable
from microninja_settings.templates import TwoButtonTemplate
#from microninja_content.api import ContentManager

wallpaper_path = "/usr/share/microninja-desktop/wallpapers/"
kano_draw_path = os.path.join('/home', username, 'Draw-content/wallpapers/')
padlock_path = "/usr/share/microninja-settings/media/Icons/padlock.png"  # needs to be 95x95
name_pattern = "-4-3.png"


class SetWallpaper(TwoButtonTemplate):
    def __init__(self, win, header=_('Choose a wallpaper'),
...
class WallpaperTable(ImageTable):
    ...
    def order_packing_of_wallpapers(self):
        ...
        for name, attributes in self.images.iteritems():
            if 'microninja-background' in name:
                self.add_to_button_list(name, True)

        for name, attributes in self.images.iteritems():
            if attributes['unlocked'] and 'microninja-background' not in name:
                self.add_to_button_list(name, True)
...
class FirstBootSetWallpaper(SetWallpaper):
    def __init__(self, win):
        SetWallpaper.__init__(self, win,
                              header=_('Ok! New desktop available for you.'),
                              subheader=_('New background unblocked!'),
                              buttons_shown=2)
