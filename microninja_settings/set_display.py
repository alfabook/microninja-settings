...
from microninja_settings.boot_config import set_config_comment, get_config_value
from microninja_settings.system.display import get_model, get_status, list_supported_modes, set_hdmi_mode, read_hdmi_mode, \
...
class SetDisplay(Template):
    def __init__(self, win):
        ...
        info_message = _('(A reboot is requested after this modify)')

        # And the current display resolution
        try:
            current_resolution = get_status()['resolution']
            info_message += _('\n\nPresent resolution:')+ ' {}'.format(current_resolution)
        except:
            pass
        ...
        flip_button = Gtk.CheckButton(_("Upturn screen"))
...
class OverscanTemplate(Gtk.Box):
    def __init__(self, win, title, description, original_overscan=None):
        ...
        self.kano_button = KanoButton(_("APPLY MODIFIES"))
        self.kano_button.connect("button-release-event", self.apply_changes)
...
class SetSimpleOverscan(OverscanTemplate):
    def __init__(self, win, original_overscan=None):
        OverscanTemplate.__init__(
            ...
            win,
            _("Overscan"),
            _("This setting permits you to change screen size."),
            original_overscan
        )
...
class SetAdvancedOverscan(OverscanTemplate):
    def __init__(self, win, original_overscan):
        OverscanTemplate.__init__(
            ...
            win,
            _("Overscan"),
            _("This setting permits you to change screen size, from border to border"),
            original_overscan
        )
